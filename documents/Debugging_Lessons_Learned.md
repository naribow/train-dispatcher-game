...
- **問題**: `drawStationLayout`メソッドが`station: Station`を引数に取っていたが、テストのために一時的に機能を無効化した場合でも、引数や依存関係の整合性を保つように注意する。
- **教訓**:
  - メソッドの引数は、そのメソッドが実際に必要とするデータのみに限定する。
  - テストやデバッグのために一時的に機能を無効化する場合でも、引数や依存関係の整合性を保つように注意する。

### 3.8. クラスインスタンスのメソッドが失われる問題 (`TypeError: segment.draw is not a function`)

- **問題**: `Map`に格納されたクラスインスタンス（例: `TrackSegment`を実装した`StraightTrack`など）を後で取り出してメソッド（例: `draw`）を呼び出そうとすると、`TypeError: segment.draw is not a function`が発生する。
- **原因**: JavaScriptのオブジェクトの参照の仕組みと、TypeScriptのコンパイル、およびモジュールバンドラー（Vite）の最適化が絡むと発生しうる。特に、クラスのメソッドがプロトタイプチェーン上に定義されている場合、オブジェクトがシリアライズ/デシリアライズされたり、バンドラーが最適化のためにプロトタイプチェーンを破壊したりすると、メソッドが失われることがある。
- **教訓**:
  - クラスのメソッドを、プロトタイプではなく**インスタンス自身のプロパティ**として定義する（例: アロー関数プロパティとして定義する）。これにより、メソッドがインスタンスに直接バインドされ、シリアライズ/デシリアライズやバンドラーの最適化の影響を受けにくくなる。
  - 複雑なオブジェクトを`Map`などに格納し、後でメソッドを呼び出す場合は、メソッドが正しく保持されているか、特に注意して確認する。

### 3.9. 線路の描画がされない問題

- **問題**: `segment.draw(track);`を呼び出しても線路が描画されない。
- **原因**: `Graphics`オブジェクトの`lineStyle`が非推奨であり、`stroke`を使用すべきだった。また、`stroke`メソッドは`Graphics`オブジェクトのメソッドチェーンの最後に呼び出す必要がある。
- **教訓**:
  - 非推奨警告は無視せず、速やかに新しいAPIに移行する。
  - `Graphics`オブジェクトの描画メソッドは、`rect()`や`circle()`などの形状定義メソッドの**後に**、`fill()`や`stroke()`などのスタイル設定メソッドを呼び出す。特に`stroke`は、`moveTo`や`lineTo`などのパス定義の後に呼び出す必要がある。
